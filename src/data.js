// const menuList = ["Trang chủ", "Tuyển sinh", "Chương trình đào tạo", "Góc sinh viên", "Tuyển dụng"];
// const menuElement = document.querySelector("#menu");
// for (let i in menuList) {
//     menuElement.innerHTML += `<li class="inline font-medium py-3 px-4 text-white  hover:text-cyan-900"><a href="">${menuList[i]}</a></li>`
// }

// =============================================
const data = [{
        "createdAt": "2022-01-10T07:26:52.549Z",
        "title": "Vinh danh 295 sinh viên FPT Polytechinc học kỳ Sping 2018",
        "img": "https://daihoc.fpt.edu.vn/media/2022/01/Quan-1-e1641111201649-373x206.png",
        "desc": "Tối ngày 16/06/2018 , Cao đẳng FPT Polytechnic Hà Nội đã tổ chức Lễ tôn vinh \"Ông vàng Poly\",Tuyên dương 295 cá nhan tập thể có thành tích xuất sác trong học tập cũng như hoạt",
        "id": "1"
    },
    {
        "createdAt": "2022-01-10T20:33:27.477Z",
        "title": "Sinh viên khối ngành kinh thế học cách \"chạm\" vào cảm xúc",
        "img": "https://daihoc.fpt.edu.vn/media/2022/01/3-1-e1641100643797-373x206.jpeg",
        "desc": "Với sự tham gia của chuyên gia trong lĩnh vực kinh doanh , chương trình đã thu hút đông đảo sinh viên Cao đẳng FPT Polytechnic tham dự . Đặc biết các bạn sinh viên bị lôi cuốn",
        "id": "2"
    },
    {
        "createdAt": "2022-01-11T03:36:13.571Z",
        "title": "Sinh viên FPoly trải nghiệm thực tế tại Mường Thanh",
        "img": "https://daihoc.fpt.edu.vn/media/2022/01/Nguye%CC%82%CC%83n-Tha%CC%80nh-Nguye%CC%82n-4-e1641113718750-373x206.png",
        "desc": "Với triết lý đào tạo \"Thực học-Thực nhiệp\", Cao đẳng FPT Polytechnic Đà Nẵng thường xuyên tổ chức cho sinh viên tham quan các đơn vị doanh nghiệp",
        "id": "3"
    }
]
export default data;